...
- Fixed legacy/`-e` extensions failing to load with `Failed to load extension: BuildMessage: Syntax Error` when they statically import a non-code asset (e.g. `import prompt from "./spawn.md" with { type: "text" }`, or a `.json`/`.html`/`.css`/`.txt` module). The compat layer's load-time rewrite hook walked the extension's import graph and forced every reached module — assets included — through a JavaScript loader, so Bun parsed the asset's contents as JS and threw a syntax error. `collectExtensionModules` now tracks only rewritable code modules (`.ts/.tsx/.mts/.cts/.js/.jsx/.mjs/.cjs`); asset modules fall through to Bun's native loader so import attributes and built-in asset loaders apply. This unbroke the `peer-coms` example extension (which imports `peer-coms-spawn.md` as text), restoring peer/agentdesk sessions that load it.
