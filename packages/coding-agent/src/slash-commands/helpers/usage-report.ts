...
export function renderAccountReports(
	...
	const freshness = latestFetchedAt
		? ` (checked ${formatCoarseDuration(Math.max(0, nowMs - latestFetchedAt))} ago)`
		: "";
		...
		for (const [index, report] of orderedReports.entries()) {
			...
			const plan =
				typeof report.metadata?.planType === "string" && report.metadata.planType
					...
					: undefined;
			const age = report.fetchedAt
				? ` - checked ${formatCoarseDuration(Math.max(0, nowMs - report.fetchedAt))} ago`
				: "";
			...
			for (const limit of report.limits) {
				...
				if (limit.window?.resetsAt !== undefined && Number.isFinite(limit.window.resetsAt)) {
					...
					const reset =
						resetAt > nowMs
							? `${limit.window.resetLabel ?? "resets"} in ${formatCoarseDuration(resetAt - nowMs)}`
		...
		for (const summary of [...disabledAccounts].sort((left, right) =>
			...
			const age =
				summary.disabledAtMs !== undefined
					? ` ${formatCoarseDuration(Math.max(0, nowMs - summary.disabledAtMs))} ago`
					: "";
