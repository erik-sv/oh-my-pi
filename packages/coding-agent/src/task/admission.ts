 ...
 *
 * Admission is a monotonic count within a host policy epoch, not a lifetime
 * permit. Callers therefore leave this gate before acquiring session or job
 * concurrency slots, which keeps recursive parent/child spawns live.
 */
export class TaskAdmission {
	...
	/** Reset the process-global gate to its fail-open default. Test-only. */
